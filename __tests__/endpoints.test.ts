...
describe("/api/brews/user/:user_id", () => {
    describe("GET /api/brews/user/:user_id", () => {
        test("GET 200 /api/brews/user/:user_id - responds with an array of brews relating to the specified user", async () => {
            const { body } = await request(app)
                .get("/api/brews/user/2")
                .expect(200);
            const { brews } = body;
